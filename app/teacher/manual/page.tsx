...
const GROUPS: ManualGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/teacher/worksheets/list', label: '출제한 학습지', icon: 'ti-files', adminOnly: true,
        summary: '전에 만든 학습지를 다시 인쇄하고, 누가 몇 점인지 봐요.',
        steps: ['「출제한 학습지」에 만든 순서대로 쌓입니다. 이름이나 시험지 코드 6자리로 찾을 수 있어요.',
          '「출력하기」를 누르면 그때 만든 그 학습지가 인쇄 화면으로 다시 열립니다 — 다시 만들 필요 없어요.',
          '줄을 누르면 그 학습지를 푼 학생 이름과 점수, 몇 개 틀렸는지가 펼쳐집니다.',
          '학생 줄 오른쪽에서 「오답」 「유사문항」 「오답＋유사」 중 하나를 누르면 그 학생 것만 새 학습지로 만들어 인쇄 화면이 열립니다.',
          '「답지·해설」은 선생님용입니다. 번호별 정답을 한 장에 모은 답지와, 해설집 풀이를 붙인 해설지를 각각 또는 함께 뽑을 수 있어요. (시험지 인쇄 화면 위쪽에도 같은 단추가 있습니다.)',
          '쎈B 문항은 해설집이 따로 없어서 쌍둥이인 쎈 문항의 풀이가 나옵니다. 주황색 칸으로 표시되고, 푸는 방법은 같지만 숫자가 달라 **풀이 끝의 답은 그 문항의 정답이 아닙니다** — 정답은 칸 맨 위에 적힌 것을 보세요. 「쌍둥이 풀이도」 체크를 끄면 빼고 뽑을 수 있어요.',
          '해설지가 너무 여러 장이면 「2단 → 3단」이나 「해설 크기 80%」로 줄이세요.',
          '문항별 정답률·유형별 오답까지 보려면 「채점결과」로 가세요.'] },
      { href: '/teacher/scan', label: 'QR 채점', icon: 'ti-qrcode',
        summary: '시험지 QR을 찍어 선생님이 직접 채점해요.',
        steps: ['「QR 채점」 → 카메라 켜기 → 시험지 오른쪽 위 QR을 비추면 채점 화면이 열려요. (카메라가 안 되면 시험지 코드 6자리를 적어도 됩니다.)',
          '맨 먼저 「누구 시험지인가요?」에서 학생을 고릅니다. 이름·담당쌤·반으로 찾을 수 있어요. 학생을 골라야 채점 화면이 열립니다 — 누구 점수인지 모르는 채로 저장되는 일이 없게요.',
          '번호 칸을 누르면 ○ → ✗ → 안 함 으로 바뀝니다. 처음에는 모두 ○ 라서 틀린 것만 누르면 돼요.',
          '「상세」를 켜면 문항마다 문제 그림·정답·출처(교재 몇 쪽 몇 번)가 같이 보입니다. (문제 그림은 이때 받아 오므로 격자만 볼 때는 훨씬 빨리 뜹니다.)',
          '서술형도 주황 「서술형」 딱지 아래에 최종 답이 나옵니다. 증명·설명 문제는 「증명 — □ABCD가 직사각형」처럼 무엇을 보여야 하는지가 적혀 있어요. (2)번에 답이 있는 문제는 「(1) 증명 (2) BC=14 cm」처럼 같이 나옵니다.',
          '「채점 저장」을 누르면 그 학생 점수로 채점결과·출제한 학습지 화면에 남습니다.'] },
      { href: '/teacher/gradings', label: '채점결과', icon: 'ti-checkbox', adminOnly: true,
        summary: '학생이 QR로 스스로 채점한 결과를 시험지별로 봐요.',
        steps: ['학생별 「틀린 문제 그대로」 「쌍둥이 문제」 「같은 유형 다른 문제」 「오답＋유사」로 재출제. 「오답＋유사」는 틀린 문제와 그 유사문제를 한 장에 이어서 뽑습니다.', '서술형은 「풀이 사진 보기」로 ○ / ✗.', '인쇄 화면 위쪽에서 「그림 크기」 「한 단에 2문항 / 3문항」 「풀이 여백」을 고를 수 있어요. 문항 그림은 전부 같은 크기로 나오고, 한 쪽에 몇 문항이 들어갈지는 문항 크기를 재서 저절로 정해집니다. 쪽 수가 많다 싶으면 「그림 크기」를 90%나 80%로, 또는 풀이 여백을 줄이세요.'] },
