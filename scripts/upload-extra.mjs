/**
 * 베이직쎈 누락분 이미지 업로드 (1,122문항 · 이미지 2,244장)
 *
 * 쓰는 법 (PowerShell) — 지난번과 똑같습니다
 *   node scripts\upload-extra.mjs "C:\Users\USER\문제은행"
 *
 * 같은 폴더에 problems_extra.json 이 있어야 합니다.
...
if (!ROOT || !fs.existsSync(ROOT)) { console.error('✗ 폴더 경로를 인자로 주세요.  예: node scripts\\upload-extra.mjs "C:\\Users\\USER\\문제은행"'); process.exit(1); }

const db = createClient(URL, KEY, { auth: { persistSession: false } });
const DATA_FILE = path.join(HERE, 'problems_extra.json');
if (!fs.existsSync(DATA_FILE)) { console.error(`✗ ${DATA_FILE} 이 없습니다. 스크립트와 같은 폴더에 problems_extra.json 을 두세요.`); process.exit(1); }
const DATA = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));

const GRADE_CODE = { '중1': 'm1', '중2': 'm2', '중3': 'm3' };
const key = (r, kind) =>
  `${GRADE_CODE[r.g]}-${r.s}/basic/${String(r.n).padStart(2, '0')}/${r.l}${kind === 'a' ? '_a' : ''}.${r.x}`;

const INDEX = new Map();
const IMG = /^\d+_\d+\)?\.(png|jpg)$/i;
