...
def split_glyphs(m, g):
    ...
    return out
NUMCOLS=[(72,95),(136,161),(412,418),(473,484),(818,838),(867,897),(1162,1169),(1222,1229)]
if __name__=='__main__':
    rows=[]; gl=[]; braw=[]
    for i in range(120):
        im=page(i); W=im.shape[1]
        gs,m,cand=groups(im)
        # 회색조로 스캔된 쪽은 색으로 못 찾는다 → 흑백으로 다시 찾되,
        # 번호가 실제로 나오는 x 열 안에 있는 것만 쓴다 (본문 글씨를 번호로 오인하지 않게)
        if len(gs)<3:
            gs2,m2,cand2=groups(im, dark=True)
            inc=lambda x: any(a0-16<=x<=b0+16 for a0,b0 in NUMCOLS)
            gs=[g for g in gs2 if inc(g[0][0])]
            cand=[g for g in cand2 if inc(g[0][0])]
            m=m2
        ...
        for g,o in brk:
            braw.append(dict(pg=i,x0=int(g[0][0]),y0=int(min(b[2] for b in g)),
                             ...
                             bx1=int(o[-1][1]),          # '[0012~0017]' 의 오른쪽 끝
                             by0=int(min(b[2] for b in g+o)), by1=int(max(b[3] for b in g+o)),
