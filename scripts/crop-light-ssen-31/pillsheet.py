...
import sys as _s; pl=json.load(open(_s.argv[4] if len(_s.argv)>4 else 'pills_body.json',encoding='utf8'))
...
for k,d in enumerate(sel, start=a):
    ...
    g=cache[d['pg']]
    x0=max(0,d['x0']-10); x1=min(g.shape[1], (830 if d["half"]==0 else 1620))
