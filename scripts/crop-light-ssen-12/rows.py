...
TITLE={1:'기본 도형',2:'위치 관계',3:'평행선',4:'작도와 합동',5:'다각형',6:'원과 부채꼴',
       7:'다면체',8:'회전체',9:'입체도형의 겉넓이와 부피',10:'도수분포표',11:'상대도수',12:'대푯값'}
rows.sort(key=lambda r:r['no'])
# 색을 못 읽은 문항('?')은 앞뒤 문항의 단계를 따른다
for i,r in enumerate(rows):
    if r['sec']!='?': continue
    prev=next((x['sec'] for x in reversed(rows[:i]) if x['sec']!='?'), None)
    nxt=next((x['sec'] for x in rows[i+1:] if x['sec']!='?'), None)
    r['sec']=prev if prev==nxt else (prev or nxt or 'B')
LEVEL={'A':1,'B':2,'S':3}
STEP={'A':'A단계','B':'B단계','S':'학교시험'}
...
for r in rows:
    ...
    ch=int(r['tcode'][:2])
    out.append(dict(b='라이트쎈',g='중1',s=2,n=ch,t=TITLE[ch],l=f"{n:04d}",
                    c=f"중1-2-{r['tcode']}",d=None,p=STEP[r['sec']],
                    e=False,v=False,i=False,x='png',
                    dir=f"중1-2/라이트쎈/{ch:02d}",lv=LEVEL[r['sec']],k='image',a=None))
...
json.dump(out,open(r'C:\Users\USER\studycheck\scripts\problems_light_m12.json','w',encoding='utf8'),ensure_ascii=False,indent=1)
base=r'C:\Users\USER\문제은행\중1-2\잘린문항_라이트쎈_중1-2'
