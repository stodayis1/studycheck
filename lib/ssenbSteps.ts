...
const SSENB_SUBCHAPTER_CONCEPT_ORDERS: Record<string, Record<number, number[]>> = {
  ...
  // 중3-2(쎈B 중등 수학 3-2, 2022년판). 2015 개정 기준 중3-2 단원(삼각비/원의 성질/통계)과
  // 쎈B의 소단원 7개가 대단원 단위로는 깔끔하게 1:1 대응하지만, "원주각" 쪽은 개념 순서와
  // 쎈B 소단원 경계가 어긋난다 - 개념23 "네 점이 한 원 위에 있을 조건"은 개념 번호상
  // 원주각(20~22) 바로 뒤에 있지만, 쎈B에서는 소단원4(원주각)이 아니라 소단원5
  // (원주각의 활용) 유형01로 다뤄져서(원에 내접하는 사각형 성질들과 함께 묶임) 23을
  // 소단원5 쪽으로 옮겨 매칭했다. 나머지는 유형 제목과 대조해 대단원 순서 그대로 나뉜다.
  '중3__2': {
    1: [1, 2, 3, 4, 5, 6, 7],   // 삼각비 - 삼각비의 값, 특수각, 예각/0·90도의 삼각비, 삼각비의 표
    2: [8, 9, 10, 11, 12],      // 삼각비의 활용 - 직각/일반 삼각형의 변의 길이, 높이, 삼각형·사각형의 넓이
    3: [13, 14, 15, 16, 17, 18, 19], // 원과 직선 - 현의 수직이등분선, 접선의 성질, 내접원, 외접사각형
    4: [20, 21, 22],             // 원주각 - 원주각과 중심각의 크기, 원주각의 성질, 호의 길이
    5: [23, 24, 25, 26],         // 원주각의 활용 - 네 점이 한 원 위에 있을 조건, 내접사각형 성질과 조건, 접선과 현이 이루는 각
    6: [27, 28, 29, 30],         // 대푯값과 산포도 - 대푯값, 편차, 분산과 표준편차
    7: [31, 32],                 // 상관관계 - 산점도, 상관관계
  },
}
