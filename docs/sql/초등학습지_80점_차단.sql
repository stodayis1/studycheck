...
begin
  if new.status = 'passed' and old.status is distinct from 'passed'
     ...
     and new.score is not null and new.score < 80
  ...
  then
    raise exception '초등 레벨학습지 80점 미만은 재도전 또는 오답유사로만 넘길 수 있어요 (예외는 원장 승인)'
      using errcode = 'P0001';
  end if;
